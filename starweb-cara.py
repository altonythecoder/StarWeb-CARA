...
def fig_animated_conjunction(sat_a, sat_b, window_hrs: int = 6, show_orbits: bool = True, show_tca: bool = True):
    ...
    now     = ts.now()
    step_min = 2
    n_frames = min(window_hrs * 30, 300)   
    trail_len = 25
    orbit_pts = 80
    ...
    # Grid lines
    r = 6371.0
    for lon_deg in range(-180, 181, 45):
        lon_rad = np.radians(lon_deg)
        lat_rad = np.linspace(-np.pi/2, np.pi/2, 40)
        x_grid = r * np.cos(lat_rad) * np.cos(lon_rad)
        y_grid = r * np.cos(lat_rad) * np.sin(lon_rad)
        z_grid = r * np.sin(lat_rad)
        fig.add_trace(go.Scatter3d(x=x_grid, y=y_grid, z=z_grid,
    ...
    fig.add_trace(go.Scatter3d(x=orb_a[0], y=orb_a[1], z=orb_a[2], mode="lines",
        line=dict(color="rgba(0,200,255,0.15)", width=1.5), 
    ...
    fig.add_trace(go.Scatter3d(x=orb_b[0], y=orb_b[1], z=orb_b[2], mode="lines",
        line=dict(color="rgba(255,107,0,0.15)", width=1.5), 
    ...
    fig.add_trace(go.Scatter3d(
        ...
        textfont=dict(color="#ff2b4d", size=9),
    ...
    def make_dynamic_traces(i):
        ...
        return [
            go.Scatter3d(x=ta[0], y=ta[1], z=ta[2], mode="lines", 
                        line=dict(color="#00c8ff", width=2.5), showlegend=False),
            go.Scatter3d(x=tb[0], y=tb[1], z=tb[2], mode="lines", 
                        line=dict(color="#ff6b00", width=2.5), showlegend=False),
            go.Scatter3d(x=[pos_a[0,i]], y=[pos_a[1,i]], z=[pos_a[2,i]], 
                        mode="markers", marker=dict(color="#00c8ff", size=9, line=dict(color="#fff",width=1)), showlegend=False),
            go.Scatter3d(x=[pos_b[0,i]], y=[pos_b[1,i]], z=[pos_b[2,i]], 
                        mode="markers", marker=dict(color="#ff6b00", size=9, line=dict(color="#fff",width=1)), showlegend=False),
            go.Scatter3d(x=[pos_a[0,i], pos_b[0,i]], y=[pos_a[1,i], pos_b[1,i]], z=[pos_a[2,i], pos_b[2,i]],
                        ...
                        text=["", f"Δ {dists[i]:.1f} km"], textfont=dict(color=dc, size=9), showlegend=False),
        ]
    ...
    for i in range(n_frames):
        title_txt = f"T+{i*step_min:04d} min | Δ {dists[i]:.1f} km"
        frames.append(go.Frame(data=make_dynamic_traces(i), traces=dyn_idx, name=str(i), 
    ...
    fig.update_layout(
        **DARK, height=640, margin=dict(l=0, r=0, t=70, b=10),
        title=dict(text=f"{sat_a.name} × {sat_b.name} — TCA: {tca_dist:.2f} km", 
                   font=dict(family="Barlow Condensed", color="#00c8ff", size=13), x=0.01),
        scene=dict(bgcolor="#000408", 
